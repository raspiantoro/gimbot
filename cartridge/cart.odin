...
load :: proc(filename: string, do_checksum:= true) -> (err: CartridgeError){
    ok: bool
    ctx.data, ok = os.read_entire_file_from_filename(filename)
    if !ok {
        return CartridgeError.ReadFileFailed
    }

    return checksum() if do_checksum else nil
}
